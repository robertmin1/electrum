...
# alice -> bob -> carol
#  /opt/wine64/drive_c/electrum-nmc/contrib/build-wine/dist/electrum-nmc-$VERSION.exe
alice="wine /opt/wine64/drive_c/electrum-nmc/contrib/build-wine/dist/electrum-nmc-$VERSION.exe --regtest -D /tmp/alice"
bob="wine /opt/wine64/drive_c/electrum-nmc/contrib/build-wine/dist/electrum-nmc-$VERSION.exe --regtest -D /tmp/bob"
carol="wine /opt/wine64/drive_c/electrum-nmc/contrib/build-wine/dist/electrum-nmc-$VERSION.exe --regtest -D /tmp/carol"
...
function wait_for_balance()
{
    ...
    cmd="wine /opt/wine64/drive_c/electrum-nmc/contrib/build-wine/dist/electrum-nmc-$VERSION.exe --regtest -D /tmp/$1"
...
if [[ $1 == "init" ]]; then
    ...
    rm -rf /tmp/$2/
    agent="wine /opt/wine64/drive_c/electrum-nmc/contrib/build-wine/dist/electrum-nmc-$VERSION.exe --regtest -D /tmp/$2"
...
if [[ $1 == "unixsockets" ]]; then
    ...
    wine /opt/wine64/drive_c/electrum-nmc/contrib/build-wine/dist/electrum-nmc-$VERSION.exe --regtest daemon -d --rpcsock=unix # Start daemon with unix domain socket
    wine /opt/wine64/drive_c/electrum-nmc/contrib/build-wine/dist/electrum-nmc-$VERSION.exe --regtest stop # Errors if it can't connect
    # Test custom socket path
    f=$(mktemp --dry-run)
    wine /opt/wine64/drive_c/electrum-nmc/contrib/build-wine/dist/electrum-nmc-$VERSION.exe --regtest daemon -d --rpcsock=unix --rpcsockpath=$f
    [ -S $f ] # filename exists and is socket
    wine /opt/wine64/drive_c/electrum-nmc/contrib/build-wine/dist/electrum-nmc-$VERSION.exe --regtest stop
    rm $f # clean up
    # Test for regressions in the ordinary TCP functionality.
    wine /opt/wine64/drive_c/electrum-nmc/contrib/build-wine/dist/electrum-nmc-$VERSION.exe --regtest daemon -d --rpcsock=tcp
    wine /opt/wine64/drive_c/electrum-nmc/contrib/build-wine/dist/electrum-nmc-$VERSION.exe --regtest stop
fi
